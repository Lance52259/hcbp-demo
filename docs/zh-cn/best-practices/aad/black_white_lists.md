...
本最佳实践将介绍如何使用Terraform创建AAD实例并配置黑白名单，以帮助您有效管理访问控制。通过将恶意IP地址加入黑名单进行封禁，将可信IP地址加入白名单进行放行，您可以灵活地控制访问来源，提升业务安全性。
...
在TF文件（如main.tf）中添加以下脚本，用于在未指定已有实例ID时创建新的DDoS高防实例：
...
**参数说明**：
- **count**：通过条件表达式 `var.instance_id == "" ? 1 : 0` 控制是否创建新实例，当未指定已有实例ID时创建。
- **ip_type**：IP类型，通过引用输入变量 instance_config 中的 ip_type 进行赋值，0表示IPv4，1表示IPv6。
- **resource_region**：资源区域，通过引用输入变量 instance_config 中的 resource_region 进行赋值，可选值包括north_china（华北）、east_china（华东）、asia_pacific（亚太）。
- **instance_access_type**：接入类型，通过引用输入变量 instance_config 中的 instance_access_type 进行赋值，0表示网站接入，1表示IP接入。
- **duration**：订购时长，通过引用输入变量 instance_config 中的 duration 进行赋值。
- **amount**：购买实例数量，通过引用输入变量 instance_config 中的 amount 进行赋值。
- **instance_name**：实例名称，通过引用输入变量 instance_config 中的 instance_name 进行赋值。
- **period_type**：订购周期类型，通过引用输入变量 instance_config 中的 period_type 进行赋值，2表示月，3表示年。
- **service_bandwidth**：业务带宽，通过引用输入变量 instance_config 中的 service_bandwidth 进行赋值。
- **basic_bandwidth**：基础带宽，通过引用输入变量 instance_config 中的 basic_bandwidth 进行赋值，当resource_region为north_china或east_china时必填。
- **elastic_bandwidth**：弹性带宽，通过引用输入变量 instance_config 中的 elastic_bandwidth 进行赋值，当resource_region为north_china或east_china时必填。
- **basic_qps**：基础QPS，通过引用输入变量 instance_config 中的 basic_qps 进行赋值，当instance_access_type为0（网站接入）时必填。
- **forwarding_rule**：转发规则数，通过引用输入变量 instance_config 中的 forwarding_rule 进行赋值，当instance_access_type为1（IP接入）时必填。
- **protected_domain**：防护域名数，通过引用输入变量 instance_config 中的 protected_domain 进行赋值，当instance_access_type为0（网站接入）时必填。
- **elastic_service_bandwidth_type**：弹性业务带宽类型，通过引用输入变量 instance_config 中的 elastic_service_bandwidth_type 进行赋值，可选值包括2（日95峰值）、3（月95峰值）。
- **elastic_service_bandwidth**：弹性业务带宽增量，通过引用输入变量 instance_config 中的 elastic_service_bandwidth 进行赋值。
- **protection_package**：防护包，通过引用输入变量 instance_config 中的 protection_package 进行赋值，可选值包括basic（保险防护）、unlimited（无限防护）。
- **enterprise_project_id**：企业项目ID，通过引用输入变量 instance_config 中的 enterprise_project_id 进行赋值。
...
**参数说明**：
- **instance_id**：通过条件表达式 `var.instance_id != "" ? var.instance_id : try(huaweicloud_aad_instance.test[0].id, "")` 进行赋值，当指定已有实例ID时使用该ID，否则引用新创建的实例ID。
- **type**：固定为"black"，表示黑名单。
- **ips**：通过引用输入变量 blacklist_ips 进行赋值，为需要封禁的IP地址列表。
...
- **ips**：通过引用输入变量 whitelist_ips 进行赋值，为需要放行的IP地址列表。
...
1. 命令行参数：`terraform apply -var="instance_config.instance_name=my-instance"`
2. 环境变量：`export TF_VAR_instance_config='{"instance_name":"my-instance"}'`
