...
DDoS高防（Advanced Anti-DDoS，AAD）是华为云提供的专业DDoS防护服务，可针对互联网服务器提供DDoS流量清洗、CC攻击防护等服务。当您的业务需要同时配置黑名单和白名单时，可以通过本实践使用Terraform创建AAD实例并配置黑白名单，实现对恶意IP的封禁和可信IP的放行，保障业务安全。
...
当您没有现成的AAD实例时，需要在TF文件（如main.tf）中添加以下脚本，创建一个新的AAD实例：
...
- **resource_region**：资源区域，可选值为`north_china`（华北）、`east_china`（华东）、`asia_pacific`（亚太）。
- **instance_access_type**：接入类型，0表示网站接入，1表示IP接入。
- **duration**：订购时长。
- **amount**：购买实例数量。
- **instance_name**：实例名称。
- **period_type**：订购周期类型，2表示月，3表示年。
- **service_bandwidth**：业务带宽，单位Mbps。
- **basic_bandwidth**：基础带宽，单位Mbps。当`resource_region`为`north_china`或`east_china`时必填，为`asia_pacific`时需为空。
- **elastic_bandwidth**：弹性带宽，单位Mbps。当`resource_region`为`north_china`或`east_china`时必填，为`asia_pacific`时需为空。
- **basic_qps**：基础QPS。当`instance_access_type`为0时必填，为1时需为空。
- **forwarding_rule**：转发规则数。当`instance_access_type`为1时必填，为0时需为空。
- **protected_domain**：防护域名数。当`instance_access_type`为0时必填，为1时需为空。
- **elastic_service_bandwidth_type**：弹性业务带宽类型，2表示日95峰值，3表示月95峰值。
- **elastic_service_bandwidth**：弹性业务带宽增量。
- **protection_package**：防护包，可选值为`basic`（保险防护）和`unlimited`（无限防护）。
- **enterprise_project_id**：企业项目ID。

### 3. 配置黑白名单

在TF文件（如main.tf）中添加以下脚本，为AAD实例配置黑名单和白名单：
...
**参数说明**：
- **instance_id**：AAD实例ID。当您已有实例时，直接指定该参数；否则将通过`huaweicloud_aad_instance.test[0].id`获取新建实例的ID。
...
# 当您已有AAD实例时，填写实例ID；否则配置instance_config创建新实例
