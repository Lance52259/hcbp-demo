...
本最佳实践将介绍如何使用Terraform创建DDoS高防实例，并为该实例配置IP黑白名单。通过将恶意IP地址加入黑名单以阻断恶意流量，将可信IP地址加入白名单以放行合法流量，从而实现对业务流量的精细化访问控制，提升业务安全性。
...
在TF文件（如main.tf）中添加以下脚本，用于创建DDoS高防实例。当未指定已有实例ID（instance_id）时，将根据instance_config配置创建新的实例。
...
**参数说明**：
- **ip_type**：IP类型，通过引用输入变量instance_config中的ip_type进行赋值，0表示IPv4，1表示IPv6。
- **resource_region**：资源区域，通过引用输入变量instance_config中的resource_region进行赋值，可选值包括north_china（华北）、east_china（华东）、asia_pacific（亚太）。
- **instance_access_type**：接入类型，通过引用输入变量instance_config中的instance_access_type进行赋值，0表示网站接入，1表示IP接入。
- **duration**：购买时长，通过引用输入变量instance_config中的duration进行赋值。
- **amount**：购买数量，通过引用输入变量instance_config中的amount进行赋值。
- **instance_name**：实例名称，通过引用输入变量instance_config中的instance_name进行赋值。
- **period_type**：购买周期类型，通过引用输入变量instance_config中的period_type进行赋值，2表示月，3表示年。
- **service_bandwidth**：业务带宽，通过引用输入变量instance_config中的service_bandwidth进行赋值。
- **basic_bandwidth**：基础带宽，通过引用输入变量instance_config中的basic_bandwidth进行赋值，当resource_region为north_china或east_china时必填，为asia_pacific时需为空。
- **elastic_bandwidth**：弹性带宽，通过引用输入变量instance_config中的elastic_bandwidth进行赋值，当resource_region为north_china或east_china时必填，为asia_pacific时需为空。
- **basic_qps**：基础QPS，通过引用输入变量instance_config中的basic_qps进行赋值，当instance_access_type为0（网站接入）时必填，为1（IP接入）时需为空。
- **forwarding_rule**：转发规则数，通过引用输入变量instance_config中的forwarding_rule进行赋值，当instance_access_type为1（IP接入）时必填，为0（网站接入）时需为空。
- **protected_domain**：防护域名数，通过引用输入变量instance_config中的protected_domain进行赋值，当instance_access_type为0（网站接入）时必填，为1（IP接入）时需为空。
- **elastic_service_bandwidth_type**：弹性业务带宽类型，通过引用输入变量instance_config中的elastic_service_bandwidth_type进行赋值，可选值包括2（日95峰值）、3（月95峰值）。
- **elastic_service_bandwidth**：弹性业务带宽增量，通过引用输入变量instance_config中的elastic_service_bandwidth进行赋值。
- **protection_package**：防护包，通过引用输入变量instance_config中的protection_package进行赋值，可选值包括basic（保险防护）、unlimited（无限防护）。
- **enterprise_project_id**：企业项目ID，通过引用输入变量instance_config中的enterprise_project_id进行赋值。

### 3. 配置黑名单

在TF文件（如main.tf）中添加以下脚本，将恶意IP地址加入黑名单，以阻断恶意流量。
...
**参数说明**：
- **instance_id**：DDoS高防实例ID，通过引用输入变量instance_id进行赋值；若未指定，则引用已创建的huaweicloud_aad_instance.test实例的ID。
- **type**：黑白名单类型，固定为"black"，表示黑名单。
- **ips**：需要加入黑名单的IP地址列表，通过引用输入变量blacklist_ips进行赋值。

### 4. 配置白名单

在TF文件（如main.tf）中添加以下脚本，将可信IP地址加入白名单，以放行合法流量。
...
**参数说明**：
- **instance_id**：DDoS高防实例ID，通过引用输入变量instance_id进行赋值；若未指定，则引用已创建的huaweicloud_aad_instance.test实例的ID。
- **type**：黑白名单类型，固定为"white"，表示白名单。
- **ips**：需要加入白名单的IP地址列表，通过引用输入变量whitelist_ips进行赋值。
...
# 方式二：使用已有实例时，配置instance_id（此时不创建新实例）
...
- [DDoS高防黑白名单最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/aad/black-white-lists)
