...
华为云AAD（Anti-DDoS）服务提供DDoS高防能力，用户可创建AAD实例并配置IP黑白名单，以阻断恶意流量并放行可信流量。本实践将指导您使用Terraform创建AAD实例（或使用已有实例），并为其配置IP黑名单和白名单，实现针对特定IP的访问控制。
...
当未指定已有实例ID（`instance_id`）时，需要创建新的AAD实例。在TF文件（如main.tf）中添加以下脚本：
...
- **count**：当`instance_id`为空时创建实例，否则不创建。
...
- **basic_bandwidth**：基础带宽（Mbps），当`resource_region`为`north_china`或`east_china`时必填，`asia_pacific`时留空。
- **elastic_bandwidth**：弹性带宽（Mbps），当`resource_region`为`north_china`或`east_china`时必填，`asia_pacific`时留空。
- **basic_qps**：基础QPS，当`instance_access_type`为0时必填，为1时留空。
- **forwarding_rule**：转发规则数，当`instance_access_type`为1时必填，为0时留空。
- **protected_domain**：防护域名数，当`instance_access_type`为0时必填，为1时留空。
- **elastic_service_bandwidth_type**：弹性业务带宽类型，可选值：2（日95峰值）、3（月95峰值）。
- **elastic_service_bandwidth**：弹性业务带宽增量。
- **protection_package**：防护包，可选值：`basic`（保险防护）、`unlimited`（无限防护）。
- **enterprise_project_id**：企业项目ID。

### 3. 配置IP黑名单

添加IP地址到黑名单以阻断恶意流量。在TF文件（如main.tf）中添加以下脚本：

```hcl
# 在指定 region 下为AAD实例配置IP黑名单
...
- **instance_id**：AAD实例ID，当指定`instance_id`时使用已有实例，否则使用新建实例的ID。
- **type**：名单类型，固定为`black`。
- **ips**：需要加入黑名单的IP地址列表。

### 4. 配置IP白名单

添加IP地址到白名单以放行可信流量。在TF文件（如main.tf）中添加以下脚本：

```hcl
# 在指定 region 下为AAD实例配置IP白名单
...
1. 命令行参数：`terraform apply -var="instance_name=my-aad"`
2. 环境变量：`export TF_VAR_instance_name=my-aad`
...
- [配置AAD实例的黑白名单最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/aad/black-white-lists)
