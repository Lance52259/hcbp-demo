...
**参数说明**：
- **count**：通过条件判断决定是否创建实例，当instance_id为空时创建1个实例，否则不创建。
- **ip_type**：IP类型，通过引用输入变量instance_config中的ip_type进行赋值。
- **resource_region**：资源区域，通过引用输入变量instance_config中的resource_region进行赋值。
- **instance_access_type**：接入类型，通过引用输入变量instance_config中的instance_access_type进行赋值。
...
- **period_type**：购买周期类型，通过引用输入变量instance_config中的period_type进行赋值。
- **service_bandwidth**：业务带宽，通过引用输入变量instance_config中的service_bandwidth进行赋值。
- **basic_bandwidth**：基础带宽，通过引用输入变量instance_config中的basic_bandwidth进行赋值。
- **elastic_bandwidth**：弹性带宽，通过引用输入变量instance_config中的elastic_bandwidth进行赋值。
- **basic_qps**：基础QPS，通过引用输入变量instance_config中的basic_qps进行赋值。
- **forwarding_rule**：转发规则数，通过引用输入变量instance_config中的forwarding_rule进行赋值。
- **protected_domain**：防护域名数，通过引用输入变量instance_config中的protected_domain进行赋值。
- **elastic_service_bandwidth_type**：弹性业务带宽类型，通过引用输入变量instance_config中的elastic_service_bandwidth_type进行赋值。
- **elastic_service_bandwidth**：弹性业务带宽增量，通过引用输入变量instance_config中的elastic_service_bandwidth进行赋值。
- **protection_package**：防护包，通过引用输入变量instance_config中的protection_package进行赋值。
- **enterprise_project_id**：企业项目ID，通过引用输入变量instance_config中的enterprise_project_id进行赋值。
- **lifecycle**：使用ignore_changes忽略部分不可更新的参数，避免因API响应中缺失这些参数而导致配置漂移。

### 3. 配置黑名单

在TF文件（如main.tf）中添加以下脚本，用于将恶意IP地址加入黑名单，以阻断恶意流量。
...
**参数说明**：
- **instance_id**：实例ID，通过引用输入变量instance_id或已创建的huaweicloud_aad_instance.test实例ID进行赋值。
- **type**：名单类型，固定为"black"，表示黑名单。
- **ips**：IP地址列表，通过引用输入变量blacklist_ips进行赋值。

### 4. 配置白名单

在TF文件（如main.tf）中添加以下脚本，用于将可信IP地址加入白名单，以放行合法流量。
