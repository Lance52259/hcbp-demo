...
云性能测试服务（Cloud Performance Test Service，CPTS）是华为云提供的性能测试服务，帮助用户模拟真实业务场景，对云上应用进行压力测试，以评估系统的性能和稳定性。

本最佳实践将介绍如何使用Terraform创建一个CPTS项目和对应的测试任务。通过本实践，您可以了解如何利用基础设施即代码（IaC）的方式快速创建和管理CPTS资源，为后续的性能测试工作奠定基础。
...
**参数说明**：
- **name**：CPTS项目的名称，通过引用输入变量 cpts_project_name 进行赋值。
- **description**：CPTS项目的描述信息，通过引用输入变量 cpts_project_description 进行赋值。
...
**参数说明**：
- **name**：CPTS测试任务的名称，通过引用输入变量 cpts_task_name 进行赋值。
- **project_id**：CPTS项目的ID，通过引用已创建的 huaweicloud_cpts_project.test 的ID进行赋值。
- **benchmark_concurrency**：CPTS测试任务的基准并发数，通过引用输入变量 cpts_task_benchmark_concurrency 进行赋值。
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建CPTS项目和测试任务
4. 运行 `terraform show` 查看已创建的CPTS项目和测试任务

## 参考信息

- [华为云云性能测试服务产品文档](https://support.huaweicloud.com/cpts/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [CPTS项目和测试任务最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/cpts/project-with-task)
