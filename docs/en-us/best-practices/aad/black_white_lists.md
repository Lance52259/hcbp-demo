...
Add the following script to the TF file (such as main.tf) to create an AAD instance. When no existing instance ID (instance_id) is specified, a new AAD instance will be created based on the instance_config configuration.
...
**Parameter Description**:
- **instance_id**: AAD instance ID, assigned by referencing the input variable instance_id or the ID of the created huaweicloud_aad_instance.test resource.
