...
AAD supports multiple access modes, including website access and IP access, to meet the protection requirements of different business scenarios. By directing business traffic to AAD high-protection nodes, AAD can detect and scrub malicious traffic in real time, forwarding only legitimate traffic to the origin server, thereby effectively defending against various network attacks such as large-scale DDoS attacks and CC attacks.

In addition, AAD provides flexible protection policies such as black/white list configuration, elastic bandwidth expansion, and protection package upgrades, helping enterprises dynamically adjust protection capabilities based on actual business needs, reducing security operation costs, and ensuring continuous and stable business operation.
...
* [Deploy Black/White Lists](black_white_lists.md) - Introduces how to use Terraform to create an AAD instance and configure IP black/white lists for it, enabling fine-grained access control over business traffic.
