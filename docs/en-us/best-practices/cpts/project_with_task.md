...
Cloud Performance Test Service (CPTS) is a performance testing service provided by Huawei Cloud, which helps users simulate real business scenarios and perform stress tests on cloud applications to evaluate system performance and stability.

This best practice will introduce how to use Terraform to create a CPTS project and a corresponding test task. Through this practice, you can learn how to use Infrastructure as Code (IaC) to quickly create and manage CPTS resources, laying a foundation for subsequent performance testing work.
...
Prepare the TF file (such as main.tf) for writing the best practice script in the specified workspace, and ensure that it (or other TF files in the same directory) contains the provider version declaration and Huawei Cloud authentication information required for deploying resources.
...
- **name**: The name of the CPTS project, assigned by referencing the input variable cpts_project_name.
- **description**: The description of the CPTS project, assigned by referencing the input variable cpts_project_description.
...
**Parameter Description**:
- **name**: The name of the CPTS test task, assigned by referencing the input variable cpts_task_name.
- **project_id**: The ID of the CPTS project, assigned by referencing the ID of the created huaweicloud_cpts_project.test.
- **benchmark_concurrency**: The benchmark concurrency of the CPTS test task, assigned by referencing the input variable cpts_task_benchmark_concurrency.
